
local GData = {}
local GData_mt = { __index = GData }

local json = require( "json" )
local lfs = require( "lfs" )
local crypto = require( "crypto" )

-------- Functions used for converting tables to strings which is used for data integrity. Functions sourced from here - http://lua-users.org/wiki/TableUtils
function table.val_to_str ( v )
	if "string" == type( v ) then
		v = string.gsub( v, "\n", "\\n" )
		if string.match( string.gsub(v,"[^'\"]",""), '^"+$' ) then
			return "'" .. v .. "'"
		end
		return '"' .. string.gsub(v,'"', '\\"' ) .. '"'
	else
		return "table" == type( v ) and table.tostring( v ) or tostring( v )
	end
end

function table.key_to_str ( k )
	if "string" == type( k ) and string.match( k, "^[_%a][_%a%d]*$" ) then
		return k
	else
		return "[" .. table.val_to_str( k ) .. "]"
	end
end

function table.tostring( tbl )
	local result, done = {}, {}
	for k, v in ipairs( tbl ) do
		table.insert( result, table.val_to_str( v ) )
		done[ k ] = true
	end
	for k, v in pairs( tbl ) do
		if not done[ k ] then
	  		table.insert( result,
			table.key_to_str( k ) .. "=" .. table.val_to_str( v ) )
		end
	end
	return "{" .. table.concat( result, "," ) .. "}"
end

local toString = function( value )
	if type( value ) == "table" then
		return table.tostring( value )
	else
		return tostring( value )
	end
end
-----------------------------------------------

--- Initiates a new GData object.
-- @param id The name of the GData to create or load ( if it already exists ).
-- @param path The path to the GData. Optional, defaults to "boxes".
-- @param baseDir The base directory for the GData. Optional, defaults to system.DocumentsDirectory.
-- @return The new object.
function GData:new( id, path, baseDir )

    local self = {}

    setmetatable( self, GData_mt )

    self.id = id
    self.path = path or "settings"
    self.baseDir = baseDir

    if self.id then
    	self:load()
    end

    return self

end

--- Loads, or reloads, this GData object from disk.
-- @param id The id of the GData object.
-- @param path The path to the GData. Optional, defaults to "boxes".
-- @param baseDir The base directory for the GData. Optional, defaults to system.DocumentsDirectory.
function GData:load( id, path, baseDir )

	-- Set up the path
	path = path or "settings"

	-- Pre-declare the new GData object
	local box

	-- If no id was passed in then assume we're working with a pre-loaded GData object so use its id
	if not id then
		id = self.id
		box = self
	end

	local data = {}

	local path = system.pathForFile( path .. "/" .. id .. ".box", baseDir or system.DocumentsDirectory )

	local file = io.open( path, "r" )

	if not file then
		return
	end

	data = json.decode( file:read( "*a" ) )
	io.close( file )

	-- If no GData exists then we are acting on a Class function i.e. not a pre-loaded GData object.
	if not box then
		-- Create the new GData object.
		box = GData:new()
	end

	-- Copy all the properties across.
	for k, v in pairs( data ) do
		box[ k ] = v
	end

	return box

end


--- Loads default settings from file.
-- @param id The name of the json file with default settings. Optional, defaults to "default".
-- @param path The path to the file. Optional, defaults to "data".
-- @param baseDir The base directory for the file. Optional, defaults to current direcroty.
function GData:loadDefaults( id, path, baseDir )

	id = id or "default"

	path = path or "settings"

	local box = self

	local data = {}

	local path = system.pathForFile( path .. "/" .. id .. ".box", baseDir or "." )

	local file = io.open( path, "r" )

	if not file then
		return
	end

	data = json.decode( file:read( "*a" ) )
	io.close( file )

	-- Copy all missed properties across.
	for k, v in pairs( data ) do
		box:setIfNew(k, v)
	end

	return box

end

--- Saves this GData object to disk.
function GData:save()

	-- Don't want this key getting saved out
	local integrityKey = self.integrityKey
	self.integrityKey = nil

	local data = {}

	-- Copy across all the properties that can be saved.
	for k, v in pairs( self ) do
		if type( v ) ~= "function" and type( v ) ~= "userdata" then
			data[ k ] = v
		end
	end

	-- Check for and create if necessary the boxes directory.
	local path = system.pathForFile( "", system.DocumentsDirectory )
	local success = lfs.chdir( path )

	if success then
		lfs.mkdir( self.path )
		path = self.path
	else
		path = ""
	end

	data = json.encode( data )

	path = system.pathForFile( self.path .. "/" .. self.id .. ".box", system.DocumentsDirectory )
	local file = io.open( path, "w" )

	if not file then
		return
	end

	file:write( data )

	io.close( file )
	file = nil

	-- Set the key back again
	self.integrityKey = integrityKey

end

--- Sets a value in this GData object.
-- @param name The name of the value to set.
-- @param value The value to set.
function GData:set( name, value )
	self[ name ] = value
	self:storeIntegrityHash( name, value )
end

--- Gets a value from this GData object.
-- @param name The name of the value to get.
-- @return The value.
function GData:get( name )
	return self[ name ]
end

--- Checks whether a value of this GData object is higher than another value.
-- @param name The name of the first value to check.
-- @param otherValue The name of the other value to check. Can also be a number.
-- @return True if the first value is higher, false otherwise.
function GData:isValueHigher( name, otherValue )
	if type( otherValue ) == "string" then
		otherValue = self:get( otherValue )
	end
	return self[ name ] > otherValue
end

--- Checks whether a value of this GData object is lower than another value.
-- @param name The name of the first value to check.
-- @param otherValue The name of the other value to check. Can also be a number.
-- @return True if the first value is lower, false otherwise.
function GData:isValueLower( name, otherValue )
	if type( otherValue ) == "string" then
		otherValue = self:get( otherValue )
	end
	return self[ name ] < otherValue
end

--- Checks whether a value of this GData object is equal to another value.
-- @param name The name of the first value to check.
-- @param otherValue The name of the other value to check. Can also be a number.
-- @return True if the first value is equal, false otherwise.
function GData:isValueEqual( name, otherValue )
	if type( otherValue ) == "string" then
		otherValue = self:get( otherValue )
	end
	return self[ name ] == otherValue
end

--- Checks whether this GData object has a specific property or not.
-- @param name The name of the value to check.
-- @return True if the value exists and isn't nil, false otherwise.
function GData:hasValue( name )
	return self[ name ] ~= nil and true or false
end

--- Sets a value on this GData object if it is new.
-- @param name The name of the value to set.
-- @param value The value to set.
function GData:setIfNew( name, value )
	if (self[ name ] == nil and self[name] ~= false) then
		self[ name ] = value
		self:storeIntegrityHash( name, value )
	end
end

--- Sets a value on this GData object if it is higher than the current value.
-- @param name The name of the value to set.
-- @param value The value to set.
function GData:setIfHigher( name, value )
	if self[ name ] and value > self[ name ] or not self[ name ] then
		self[ name ] = value
		self:storeIntegrityHash( name, value )
	end
end

--- Sets a value on this GData object if it is lower than the current value.
-- @param name The name of the value to set.
-- @param value The value to set.
function GData:setIfLower( name, value )
	if self[ name ] and value < self[ name ] or not self[ name ] then
		self[ name ] = value
		self:storeIntegrityHash( name, value )
	end
end

--- Increments a value in this GData object.
-- @param name The name of the value to increment. Must be a number. If it doesn't exist it will be set to 0 and then incremented.
-- @param amount The amount to increment. Optional, defaults to 1.
function GData:increment( name, amount )
	if not self[ name ] then
		self:set( name, 0 )
	end
	if self[ name ] and type( self[ name ] ) == "number" then
		self[ name ] = self[ name ] + ( amount or 1 )
		self:storeIntegrityHash( name, value )
	end
end

--- Decrements a value in this GData object.
-- @param name The name of the value to decrement. Must be a number. If it doesn't exist it will be set to 0 and then decremented.
-- @param amount The amount to decrement. Optional, defaults to 1.
function GData:decrement( name, amount )
	if not self[ name ] then
		self:set( name, 0 )
	end
	if self[ name ] and type( self[ name ] ) == "number" then
		self[ name ] = self[ name ] - ( amount or 1 )
		self:storeIntegrityHash( name, value )
	end
end

--- Clears this GData object.
function GData:clear()
	for k, v in pairs( self ) do
		if k ~= "integrityControlEnabled"
			and k ~= "integrityAlgorithm"
			and k ~= "integrityKey"
			and k ~= "id"
			and type( k ) ~= "function" then
				self[ k ] = nil
		end
	end
end

--- Deletes this GData object from disk.
-- @param id The id of the GData to delete. Optional, only required if calling on a non-loaded object.
function GData:delete( id )

	-- If no id was passed in then assume we're working with a pre-loaded GData object so use its id
	if not id then
		id = self.id
	end

	local path = system.pathForFile( self.path, system.DocumentsDirectory )

	local success = lfs.chdir( path )

	os.remove( path .. "/" .. id .. ".box" )

	if not success then
		return
	end

end

--- Enables or disables the Syncing of this box.
-- @param enabled True if Sync should be enabled, false otherwise.
function GData:setSync( enabled, id )

	-- If no id was passed in then assume we're working with a pre-loaded GData object so use its id
	if not id then
		id = self.id
	end

	native.setSync( self.path .. "/" .. id .. ".box", { iCloudBackup = enabled } )

end

--- Checks if Syncing for this box is enabled or not.
-- @param enabled True if Sync is enabled, false otherwise.
function GData:getSync( id )

	-- If no id was passed in then assume we're working with a pre-loaded GData object so use its id
	if not id then
		id = self.id
	end

	return native.getSync( self.path .. "/" .. id .. ".box", { key = "iCloudBackup" } )

end

--- Enables integrity checking.
-- @param algorithm The hashing algorithm to use, see this page for possibles - http://docs.coronalabs.com/api/library/crypto/index.html
-- @param key The seed to use for the hashing algorithm.
function GData:enableIntegrityControl( algorithm, key )
	self.integrityAlgorithm = algorithm
	self.integrityKey = key
	self.hash = self.hash or {}
	self.integrityControlEnabled = true
end

--- Disables integrity checking.
function GData:disableIntegrityControl()
	self.integrityAlgorithm = nil
	self.integrityKey = nil
	self.hash = nil
	self.integrityControlEnabled = false
end

--- Verifies that the passed in value matches the stored hash.
-- @param name The name of the value to check.
-- @param value The value to check.
-- @return True if the value matches the hash false otherwise.
function GData:verifyItemIntegrity( name, value )
	-- just hash the tostring() version and compare against that!
	if toString( value ) then
		local generatedHash = crypto.hmac( self.integrityAlgorithm, toString( value ), self.integrityKey, false )
		local storedHash = self.hash[ name ]
		return generatedHash == storedHash
	end
end

--- Stores the hash value of the given value to be used for integrity checks.
-- @param name The name of the value to set.
-- @param value The value to set. Optional, if not included will just pull the value from the name supplied.
function GData:storeIntegrityHash( name, value )

	if not self.integrityControlEnabled then
		return
	end

	value = value or self[ name ]

	self.hash = self.hash or {}

	if value then
		self.hash[ name ] = crypto.hmac( self.integrityAlgorithm, toString( value ), self.integrityKey, false )
	end

end

--- Updates/sets the hash value of the all stored values.
function GData:updateAllIntegrityHashes()

	for k, v in pairs( self ) do
		if k ~= "integrityControlEnabled"
			and k ~= "integrityAlgorithm"
			and k ~= "integrityKey"
			and k ~= "hash"
			and k ~= "id"
			and  toString( v ) then
				self:storeIntegrityHash( k, v )
		end
	end

end

--- Checks the hashed versions of all stored data. Will remove any values that don't match their hashes, i.e. they've been tampered with.
function GData:verifyIntegrity()

	if not self.integrityControlEnabled then
		return
	end

	local corruptEntries = {}

	for k, v in pairs( self ) do
		if k ~= "integrityControlEnabled"
			and k ~= "integrityAlgorithm"
			and k ~= "integrityKey"
			and k ~= "hash"
			and k ~= "id"
			and k ~= "path"
			and toString( v ) then

				if not self:verifyItemIntegrity( k, v ) then
					corruptEntries[ #corruptEntries + 1 ] = { name = k, value = v }
					self[ k ] = nil
					self.hash[ k ] = nil
				end
		end
	end

	for k, v in pairs( self.hash ) do
		if not self[ k ] then
			corruptEntries[ #corruptEntries + 1 ] = { name = k, value = v }
			self[ k ] = nil
			self.hash[ k ] = nil
		end
	end

	return corruptEntries

end

--- Gets the path to the stored file. Useful if you want to upload it.
-- @return Two paramaters; the full path and then the relative path.
function GData:getFilename()
	local relativePath = self.path .. "/" .. self.id .. ".box"
	local fullPath = system.pathForFile( relativePath, system.DocumentsDirectory )
	return fullPath, relativePath
end

--- Destroys this GData object.
function GData:destroy()
	self:clear()
	self = nil
end

return GData
